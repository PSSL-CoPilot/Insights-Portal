...
function DataBanner() {
  ...
  return (
    <div className="flex items-start gap-3 border-b border-warn/30 bg-warn-soft px-5 py-2.5 text-[13px] text-[#7a5200] sm:px-8">
      <AlertTriangle className="mt-0.5 size-4 shrink-0" />
      <div className="flex-1">
        <strong>Data warning:</strong> {bad.length} issue{bad.length > 1 ? "s" : ""} found while reading the workbook — e.g. <em>[{bad[0].sheet}] {bad[0].message}</em>{" "}
...
function MobileNav() {
  ...
  return (
    <nav className="flex gap-1.5 overflow-x-auto border-b border-line bg-white px-4 py-2 lg:hidden" aria-label="Primary">
      {NAV.map((n) => {
        const on = n.href === "/" ? path === "/" : path.startsWith(n.href);
        return (
          <Link key={n.href} href={n.href} className={cn("flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[12.5px] font-semibold", on ? "bg-ink text-white" : "bg-[#f0f0eb] text-mute")}>
